...
struct CheckIn: Codable {
    ...
    var isEmpty: Bool {
        ...
        restingHR == nil && avgDailySteps == nil && sleepScore == nil && notes.isEmpty
    }
}
...
struct DayLog: Codable {
    ...
    /// Convenience for checkbox-style UIs.
    /// Backed by `exerciseDone` so we don’t change the persisted schema.
    var completedExerciseIDs: [String] {
        get {
            exerciseDone.compactMap { (key, value) in value ? key : nil }
        }
        set {
            var map: [String: Bool] = [:]
